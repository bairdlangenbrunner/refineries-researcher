"""Export the union master to a review xlsx (a read-only snapshot, not a batch deliverable).

    python scripts/export_master.py                 # latest master -> batches/refineries_master_<stamp>_worldwide_export.xlsx
    python scripts/export_master.py --master data/master_YYYYMMDD_HHMM_ET.parquet
...
edits the master — it just renders it for review.
"""
...
from paths import BATCHES, latest_master, ordered_columns

try:
    import pandas as pd
except ImportError:  # pragma: no cover
    sys.exit("export_master.py needs pandas + openpyxl (pip install -r requirements.txt)")

DROP_COLS = ["RefineryID"]   # internal id — never emitted (workbook_conventions.md)


def _stamp_from(master: Path) -> str:
    # master_20260713_1008_ET.parquet -> 20260713_1008_ET
    return master.stem[len("master_"):]


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--master", help="path to a master_*.parquet (default: latest)")
    ap.add_argument("--out", help="output xlsx (default: batches/refineries_master_<stamp>_worldwide_export.xlsx)")
    args = ap.parse_args()
    ...
    stamp = _stamp_from(master)
    out = Path(args.out) if args.out else (BATCHES / f"refineries_master_{stamp}_worldwide_export.xlsx")
    ...
    summary = pd.DataFrame({
        "Metric": ["Master", "Refineries", "Countries", "With capacity (kbpd)",
                   "With coordinates", "Multi-source rows", "Single-source rows"],
        "Value": [master.name, len(df), int(df["Country"].nunique()),
